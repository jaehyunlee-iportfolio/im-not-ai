...
**개조식 보고서 가드 (genre=리포트/공적):** 명사형 종결(체언 종결 "…확정/…필요/…구축", "~함·~음·~임")은 보고 문건의 **register이지 AI 티가 아니다.** 장르가 리포트·공적(개조식)으로 추정·지정되면 명사형 종결을 "~한다/~했다" 평서형으로 전환하지 않는다. E-2·F-4·I 처방은 이 경우 종결 양식 자체에는 적용 제외(어휘·중복·피동 등 다른 처방은 그대로 적용). 칼럼·블로그·에세이에는 해당 없음.

**회사 문서 개조체 정규화 (genre=회의록/보고서/기획서, 기본 ON):** 위 문서들의 표준 종결은 개조체(명사형 종결: ~함, ~음, ~임, ~할 것, ~예정, ~필요)다. 입력이 "~한다/~했다/~이다" 평서체로 와도 이 장르에서는 **문장 종결을 개조체로 정규화한다**(위 register 보존 원칙의 명시적 예외 — 보존만 하던 기존 가드의 반대 방향). 의미, 수치, 고유명사, 인용은 그대로 두고 종결 형태만 바꾼다: 진행한다→진행, 정한다→정함/결정, 배포한다→배포/배포 예정, 어렵다→어려움, 복잡하다→복잡함, 필요하다→필요, 했다→함/완료. 헤딩, 표 셀, 라벨, 날짜는 변환 대상 아님. 입력이 합쇼체("~습니다") 격식 공문이면 그 register를 따른다(개조체 강제 아님).
...
| E-2 | 동일 종결어미 "~다" 4문장 연속 + 진행형 "~고 있다" 자동 매핑 | S2 | "~었다·~ㄴ다·~는다·~기 마련이다·~ㄹ 것이다" 등 다양화. "~고 있다" 단순 시제 환원 가능 시 환원("읽고 있다" → "읽는다"). **단, 개조식 보고서(genre=리포트/공적)의 명사형 종결은 보존 — 평서형 전환·강제 변주 금지** |
...
| ID | 패턴 | 심각도 | 처방 |
|---|---|---|---|
| F-4 | 한자어 명사화 -성/-적/-화 + 영어 명사화 -tion/-ment/-ness/-ity 누적 (한 글 12회+) | S2 | 동사·형용사 어근으로 환원("the implementation of the policy" → "정책 시행" 또는 "정책을 시행하기"). **단, 개조식 보고서의 명사형 종결 자체는 환원 대상 아님(register)** |
...
4. **register 보존**: 원문 격식체면 결과도 격식체. 평어체로 떨어뜨리지 않는다. **개조식 보고서(리포트/공적)면 명사형 종결을 ~한다 평서형으로 바꾸지 않았는가. 회의록/보고서/기획서면 반대로 평서체(~한다)를 개조체(~함/~음)로 정규화했는가**
