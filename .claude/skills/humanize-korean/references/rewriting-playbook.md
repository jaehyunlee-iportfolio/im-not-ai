...
- **개조식 보고서 예외 (genre=리포트/공적)**: 명사형 종결("…확정/…필요/…구축", "~함·~음·~임")은 보고 문건의 register다. 이 장르에서는 명사형 종결을 "~한다/~했다" 평서형으로 전환하거나 강제 변주하지 않는다. 리듬은 문장 길이·구조로만 조절. (칼럼·블로그·에세이에는 해당 없음.)
- **회사 문서 개조체 정규화 (genre=회의록/보고서/기획서)**: 위 보존 규칙의 확장(반대 방향). 이 문서들은 표준 종결이 개조체이므로, 입력이 평서체("~한다/~했다")여도 종결을 개조체(~함/~음/~임/~예정/~필요)로 정규화한다. 의미는 그대로 두고 종결 형태만 변환(진행한다→진행, 정한다→정함/결정, 어렵다→어려움). 합쇼체("~습니다") 격식 공문은 예외.
...
- **개조식 보고서 예외 (genre=리포트/공적)**: "…필요/…요(要)" 같은 명사형 종결이 보고 문건의 정상 양식이면 평서형으로 풀지 않는다. 형식명사 처방은 본문 산문에만 적용.
